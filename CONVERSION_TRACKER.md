...
- **Damage number easing curves**: Pop-in animation now uses Godot's `ease()` function (ease-out cubic for the rise to peak, ease-out quartic for the settle) instead of linear interpolation. Gives damage numbers a more decisive pop and softer landing, matching the juice style of dash squash.
