...
- **Game Modes** — 9 selectable game modes (Normal, Endless, Boss Rush, Speedrun, PvP, Survival, Gauntlet, Boss Gauntlet, Daily Challenge) chosen on the main menu; Endless escalates waves every 30s, Boss Rush fights all 5 bosses back-to-back with timer + full heal between, Speedrun tracks biome splits + personal best with a timer HUD, PvP is local 1v1 arena combat, Survival is no-healing one-life, Gauntlet is sequential biome kill challenges, Boss Gauntlet is every boss in sequence with no healing, **Daily Challenge** is a deterministic seed-based run with fixed modifiers for all players on the same day (one attempt per day, historical leaderboard, gold HUD overlay); mode persists to disk
