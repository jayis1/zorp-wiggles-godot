...
- **Phase 31**: Quality of life (FPS counter, minimap zoom, auto-save, colorblind modes, tutorial, tooltips, ping system, UI scaling, color filters) — **COMPLETE** (10/10: FPS counter + performance overlay [F3] with 60-point sparkline graph and color-coded FPS, minimap scroll-wheel zoom 40-400 world units with zoom indicator, ping system [middle mouse] with 4 ping types [default/danger/loot/nav] + 3D beacons + minimap diamonds + edge arrows, UI scaling [F8/Shift+F8] 75-150% with proportional HUD element + font scaling, colorblind correction modes [F7] for protanopia/deuteranopia/tritanopia/achromatopsia via 3×3 color transform shader, cosmetic color filters [F6] sepia/noir/thermal/x-ray via screen-space shader, auto-save system [every 30s + on biome change] with save/load to `user://zorp_save.json` + "Continue" button on main menu, control rebinding [28 rebindable P1 actions] with click-to-listen mode + conflict resolution + `user://zorp_rebind.json` persistence, tutorial mode [12 contextual steps triggered by gameplay events] with dismissible tooltips + `user://zorp_tutorial.json` persistence + replay from settings, tooltip system [hover over collectibles/enemies] with 0.4s delay + auto-positioning + rarity/stats display)
- **Phase 32**: Multiplayer & social (local leaderboards, ghost mode, replay system, challenge seeds, PvP arena) — **IN PROGRESS** (5/8: local leaderboards [F12] with per-mode rankings + top-20 entries + gold/silver/bronze medals + challenge seed panel, ghost mode [Shift+G] races a translucent cyan ghost of your best run with ahead/behind delta indicator, replay system [F11 browser] records full runs at 30Hz + plays back with a ghost player + auto-prunes to 10 saved replays, challenge seeds [shareable "ZW-<seed>-<checksum>" strings] with clipboard copy + validation + apply, PvP arena mode [Zorp vs Zerp, best-of-3, 90s rounds, HP bars + round wins HUD, enemy spawning disabled, projectile PvP damage routing])
- **Phase 33**: Procedural content (quests, enemy variants, weapon mod fusions, world modifiers) — **IN PROGRESS** (4/8: procedural quest system with 16 objective types + 4 modifiers that scale with player level/biome and appear in the Quest Log under a "✦ PROCEDURAL QUESTS" section; enemy variant system with 3 tiers [Elite/Golden/Champion] × 12 random traits [Swift/Tanky/Berserker/Regenerating/Exploding/Shielded/Lifesteal/Teleporting/Venomous/Knockback-Immune/Evasive/Giant] that promote regular enemies on spawn, with per-tier visuals [color blend, scale tween, glow light] + trait ticking [regen/teleport/berserker/venom] + Exploding death AoE + Champion bonus rare loot; weapon mod fusion system — fuse 2 discovered mods into a procedural fused mod [e.g. "Ultra Nova" = Homing Laser + Plasma Nova] with averaged stats + 0-30% random bonus, alternates between both parents' projectile behaviors on each shot, 6-slot registry persists to disk, fusion menu [⚗ button in crafting menu] with Equip/Delete; world modifier system with 20 per-run modifiers [Double Trouble/Glass Cannon/No Dash/Swift Foes/Extra Loot/Double XP/Hardened/Pacifist Fog/Slow-mo World/Lightweight/Berserker/Regen Rain/Famine/Champion Enemies/Weather Chaos/Big Head Mode/Thin Skin/Vampire/No Pet/Double Bosses/Treasure Hoarder] rolled deterministically from the world seed, 2-4 per run, integrated across spawner/player/weather/enemy systems; remaining: procedural dungeons, procedural biomes, procedural music, procedural boss generation)
