...
func _make_pattern_script(patterns: Array, _base_damage: int) -> GDScript:
	...
	src += "const SW_SCENE := preload(\"res://scenes/entities/shockwave.tscn\")\n"
	src += "const PROJ_SCENE := preload(\"res://scenes/entities/enemy_projectile.tscn\")\n"
	src += "const BLOB_SCENE := preload(\"res://scenes/entities/enemy_blob.tscn\")\n"
	...
	src += "	var pre_charge_speed = boss.speed\n"
	src += "	boss.speed *= 2.0\n"
	...
	src += "	if not _player:\n"
	src += "		return\n"
	src += "	# Teleport behind the player (opposite of camera forward direction).\n"
	src += "	# The player body never rotates — the camera rig handles facing — so\n"
	src += "	# we must use the camera's basis, not the player's basis, to determine\n"
	src += "	# 'behind.' In Godot 4, basis.z points backward, so -basis.z is forward.\n"
	src += "	var behind: Vector3\n"
	src += "	var cam = get_viewport().get_camera_3d()\n"
	src += "	if cam:\n"
	src += "		var fwd = -cam.global_basis.z\n"
	src += "		fwd.y = 0\n"
	src += "		fwd = fwd.normalized()\n"
	src += "		behind = _player.global_position - fwd * 4.0\n"
	src += "	else:\n"
	src += "		behind = _player.global_position + Vector3(0, 0, 4.0)\n"
	...
	src += "	# The boss body never rotates, so we compute 'forward' as the direction\n"
	src += "	# from boss to player, then check if the player is close enough.\n"
	src += "	if not _player:\n"
	src += "		return\n"
	src += "	var to_player = (_player.global_position - boss.global_position).normalized()\n"
	src += "	# Use the direction from boss to player as the beam axis — the boss\n"
	src += "	# always faces the player for this attack, so any direction is 'forward'.\n"
	src += "	if _player.global_position.distance_to(boss.global_position) < 20.0:\n"
