...
@onready var hp_bar: SubViewport = $HpBarViewport  # 3D HP bar via viewport
...
func _physics_process(delta: float) -> void:
	...
	if spawn_grace_timer > 0:
		...
		# During grace: just stand there, fading in
...
func _try_attack(player: Node3D) -> void:
	...
	# Brief windup
	windup_tween.tween_property(self, "scale", Vector3.ONE * base_scale * (1.0 - GameConstants.ENEMY_ATTACK_WINDUP_SQUASH), GameConstants.ENEMY_ATTACK_WINDUP_TIME)
	windup_tween.tween_callback(_execute_attack.bind(player))

func _executeAttack(player: Node3D) -> void:
	is_windup = false
	# Deal damage
	...
	lunge_dir.y = 0
	var lunge_mult := clampf(base_scale / GameConstants.ENEMY_ATTACK_LUNGE_SIZE_BASE, GameConstants.ENEMY_ATTACK_LUNGE_SIZE_MULT_MIN, 1.0)
...
func _executeAttack(_player: Node3D) -> void:
	pass  # Override in subclasses for special attacks
...
func take_damage(amount: int) -> void:
	...
	if body_mesh:
		...
		flash_tween.tween_property(body_mesh.material_override, "albedo_color", Color.WHITE, 0.05)
		flash_tween.chain().tween_property(body_mesh.material_override, "albedo_color", base_color, 0.15)
...
func _update_visuals(delta: float) -> void:
	...
	if hp_bar:
		...
		if ratio > 0.5:
			var t := (ratio - 0.5) * 2.0
			bar_color = Color(1.0 - t, 1.0, 0.0)
		else:
			var t := ratio * 2.0
			bar_color = Color(1.0, t, 0.0)
		# Update the HP bar color via shader or direct material
	...
	if ratio < 0.25 and ratio > 0:
		...
		if body_mesh:
			...
			body_mesh.material_override.albedo_color = warning_color